...
def build_faiss_index(fingerprints):
    f = len(fingerprints[0])
...
def process_and_upload_similarity_scores(input_df, molecule_structures, faiss_index, bucket_name):
    for chembl_id in input_df["chembl_id"].unique():
        ...
        result_file = f"similarity_scores_{chembl_id}.parquet"
        save_to_parquet(similarity_scores_df, result_file)
        upload_file_to_s3(result_file, bucket_name, f"final_task/gaplanyan_anna/{os.path.basename(result_file)}")


def build_faiss_index_parallel(chunks):
    with Pool(cpu_count()) as pool:
        indices = pool.map(build_faiss_index, chunks)
...
if __name__ == "__main__":
    ...
    molecule_structures = load_and_clean_data("input_files/molecule_structures.csv", has_smiles=True)
    ...
    for chembl_id in all_molecules_df["chembl_id"].unique():
        logging.info(f"Computing similarity scores for {chembl_id}...")
        molecule_df = all_molecules_df[all_molecules_df["chembl_id"] == chembl_id]
        similarity_scores_df = process_similarity_query(molecule_df, all_molecules_df, faiss_index)
